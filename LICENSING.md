# Licensing

AX Serving is offered under a dual-license model:

1. `AGPL-3.0-only` for the community/open-source option
2. Separate commercial terms from DEFAI Private Limited

## Open-Source Option

Unless you have a valid commercial license, your rights to use, modify, and
distribute this repository are governed by the GNU Affero General Public
License, version 3 only.

See:

- [LICENSE](LICENSE)
- [LICENSE-AGPL.md](LICENSE-AGPL.md)

## Commercial Option

If you need to embed AX Serving in a proprietary product, offer it under
closed-source terms, or otherwise operate outside AGPL obligations, you need a
separate commercial license.

See:

- [LICENSE-COMMERCIAL.md](LICENSE-COMMERCIAL.md)

## Public Contributions

This repository does not currently accept unsolicited public code
contributions.

Public issue reports are welcome. Code changes are handled by the maintainers
or by separately approved contributors under written terms.

See:

- [CONTRIBUTING.md](CONTRIBUTING.md)

## Deployment Reminder

If you modify AX Serving and make it available for remote network interaction,
the AGPL has additional source-availability requirements. Review section 13 of
the AGPL carefully before deploying modified hosted versions.
