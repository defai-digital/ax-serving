...
use ax_serving_protocol::{
    ADMISSION_STATE_HEADER, ATTEMPT_ID_HEADER, AttemptId, DISPATCH_TOKEN_HEADER, DomainId,
...
/// Runtime-SDK-free configuration shared by the Mac runtime agent and Dynamo
/// domain adapter.
#[derive(Clone, Debug)]
pub struct ProxyConfig {
    ...
    pub expected_domain_id: Option<DomainId>,
...
struct DispatchAuthState {
    ...
    expected_domain_id: Option<DomainId>,
    require_dispatch_identity: bool,
}
...
fn is_successful_event_stream(status: StatusCode, content_type: &str) -> bool {
    status.is_success()
        && content_type
            .to_ascii_lowercase()
...
pub fn router(
    ...
    let state = ProxyState {
        ...
        upstream_health_path: config.upstream_health_path,
        inflight,
    ...
    let dispatch_auth = DispatchAuthState {
        token: config.dispatch_token.map(Arc::from),
        expected_domain_id: config.expected_domain_id,
        require_dispatch_identity: config.require_dispatch_identity,
    };
...
async fn dispatch_auth_middleware(
    ...
    } else if !state.require_dispatch_identity {
    ...
    if let Some(expected_domain_id) = &state.expected_domain_id {
        let domain_matches = request
            .headers()
            .get("x-ax-domain-id")
            .and_then(|value| value.to_str().ok())
            .and_then(|value| value.parse::<DomainId>().ok())
            .is_some_and(|candidate| candidate == *expected_domain_id);
        if !domain_matches {
            return (
                StatusCode::BAD_REQUEST,
                axum::Json(serde_json::json!({
                    "error": {
                        "code": "AXS_DISPATCH_DOMAIN_MISMATCH",
                        "message": "dispatch domain does not match this adapter"
                    }
                })),
            )
                .into_response();
...
async fn health(State(state): State<ProxyState>) -> axum::response::Response {
    ...
    let request = state
        ...
        .get(format!(
            "{}{}",
            state.upstream_url, state.upstream_health_path
        ))
...
async fn proxy_to(
        ...
        .is_some_and(|ready| !ready.load(Ordering::Acquire))
    {
        return pre_admission_response(
            StatusCode::SERVICE_UNAVAILABLE,
            "AXS_DOMAIN_NOT_READY",
            "execution domain is not ready for new requests",
            client_headers,
        );
    }
    ...
    match req.headers(current_trace_headers()).body(body).send().await {
        Ok(resp) => {
            ...
            let content_type = resp_headers
                ...
                .to_string();
            let is_event_stream = is_successful_event_stream(status, &content_type);
...
mod tests {
    ...
    #[test]
    fn event_streaming_requires_a_success_status() {
