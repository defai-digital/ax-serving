...
//! # Error handling
//!
//! Inference messages use at-most-one delivery. JetStream redelivery cannot
//! prove that a runtime did not admit a request, so duplicate delivery is
//! forbidden for this compatibility transport.
...
pub struct NatsConfig {
    ...
    /// Delivery count. Inference safety requires this to remain exactly one.
    pub max_deliver: i64,
...
impl NatsConfig {
    ...
    pub fn try_from_env() -> anyhow::Result<Self> {
        ...
        if let Some(n) = env_parse::<i64>("AXS_NATS_MAX_DELIVER")? {
            if n != 1 {
                anyhow::bail!(
                    "AXS_NATS_MAX_DELIVER must be 1; inference requests cannot be safely redelivered"
                );
            }
            cfg.max_deliver = 1;
        }
...
pub struct NatsRequest {
    ...
    pub body_hex: String,
    /// Deprecated wire field accepted for decode compatibility and ignored by workers.
...
impl NatsDispatcher {
    ...
    /// Total number of transport-level reroute events.
    ...
    /// Forward a client request to a worker via NATS JetStream.
    ///
    /// The legacy authorization parameter is ignored. Public credentials
...
mod tests {
    ...
    #[test]
    fn nats_config_rejects_inference_redelivery() {
        let _guard = crate::test_env::lock();
        unsafe { std::env::set_var("AXS_NATS_MAX_DELIVER", "2") };
        let error = NatsConfig::try_from_env().unwrap_err().to_string();
        unsafe { std::env::remove_var("AXS_NATS_MAX_DELIVER") };

        assert!(error.contains("cannot be safely redelivered"));
    }
    ...
    #[test]
    fn nats_request_never_serializes_legacy_authorization() {
        let req = NatsRequest {
